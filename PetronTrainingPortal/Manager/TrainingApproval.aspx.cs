...
public partial class Manager_TrainingApproval : System.Web.UI.Page
{
        ...
        using (var context = new DatabaseContext())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    ReloadCapacity(training.TrainingId);
                }
            }
    ...
    public void ReloadCapacity(int id)
    {
        using (var context = new DatabaseContext())
        {
            List<CapacityRemainingViews> capacityView = new List<CapacityRemainingViews>();
            capacityView.Clear();
            ...
            {
                var dept = context.Departments.FirstOrDefault(c => c.DepartmentId == user.DepartmentId);
                var capacity = context.TrainingCapacities.FirstOrDefault(c => c.TrainingId == id && c.DepartmentId == dept.DepartmentId);
                capacityView.Add(new CapacityRemainingViews()
                {
                    MaxNumber = capacity.Capacity.ToString(),
                    Remaining = (capacity.Capacity - capacity.CurrentCount).ToString()
                });

                gridCapacity.DataSource = null;
                gridCapacity.DataSource = capacityView.ToList();
                gridCapacity.DataBind();
            }
        }
    }
