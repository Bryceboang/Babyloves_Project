    ...
    void ReloadCode(int id, string Code)
    {
        try
        {
            using (var context = new DatabaseContext())
            {
                ...
                var shopTraining = context.ShopTrainings.FirstOrDefault(c => c.ShopTrainingId == Variables.shopTrainingId);
                if (shopTraining != null)
                {
                    if (shopTraining.IsSubmitted == true)
                    {
                        this.gridNominee.Columns[4].Visible = false;
                    }
                    else
                    {
                        this.gridNominee.Columns[4].Visible = true;
                    }
                }
